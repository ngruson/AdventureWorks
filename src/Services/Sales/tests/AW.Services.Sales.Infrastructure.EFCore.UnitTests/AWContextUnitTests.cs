...
namespace AW.Services.Sales.Infrastructure.EFCore.UnitTests
{
    public class AWContextUnitTests
    {
        ...
        )
        {
            ...
            entityTypes[6].Name.Should().Be(typeof(SalesOrderLine).FullName);
            entityTypes[7].Name.Should().Be(typeof(SalesOrderSalesReason).FullName);
            entityTypes[8].Name.Should().Be(typeof(SalesPerson).FullName);
            entityTypes[9].Name.Should().Be(typeof(SalesPersonEmailAddress).FullName);
            entityTypes[10].Name.Should().Be(typeof(SalesPersonPhone).FullName);
            entityTypes[11].Name.Should().Be(typeof(SalesReason).FullName);
            entityTypes[12].Name.Should().Be(typeof(SpecialOffer).FullName);
            entityTypes[13].Name.Should().Be(typeof(SpecialOfferProduct).FullName);
            entityTypes[14].Name.Should().Be(typeof(StoreCustomer).FullName);
        }
    }
}